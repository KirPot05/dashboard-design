...
function DataTable({ columns }) {
  return (
    <div className="w-full mt-5">
      {/* Table header */}
      <div className="flex items-center justify-between text-sm my-2 px-5">
        {columns.map((col, idx) => (
      ...
      {Array.from({ length: 6 }).map((_, id) => (
        <div
          key={id}
          className="py-3 px-5 flex items-center justify-between font-semibold text-sm even:bg-slate-100"
        >
